...
*/
  ...
  class bm_authors {
      ...
      if ($number_of_author_rows = tep_db_num_rows($authors_query)) {
        if ($number_of_author_rows <= MAX_DISPLAY_AUTHORS_IN_A_LIST) {
          ...
          while ($authors = tep_db_fetch_array($authors_query)) {
            ...
            $content .= '<li><a href="' . tep_href_link('articles.php', 'authors_id=' . $authors['authors_id']) . '">' . $authors_name . '</a></li>';
          }
          $content .= '</ul>';
        } else {
          // Display a drop-down
          ...
          if (MAX_AUTHORS_LIST < 2) {
            $authors_array[] = ['id' => '', 'text' => PULL_DOWN_DEFAULT];
          }

          while ($authors = tep_db_fetch_array($authors_query)) {
            $authors_name = ((strlen($authors['authors_name']) > MAX_DISPLAY_AUTHOR_NAME_LEN) ? substr($authors['authors_name'], 0, MAX_DISPLAY_AUTHOR_NAME_LEN) . '..' : $authors['authors_name']);
            $authors_array[] = ['id' => $authors['authors_id'], 'text' => $authors_name];
          }

          $content = tep_draw_form('authors', tep_href_link('articles.php'), 'get') .
                     tep_draw_pull_down_menu('authors_id', $authors_array, ($_GET['authors_id'] ?? ''), 'onchange="this.form.submit();" size="' . MAX_AUTHORS_LIST . '" style="width: 100%"') . tep_hide_session_id() .
                     '</form>';
        }
      }
    ...
    function install() {
      ...
      tep_db_query("insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_BOXES_AUTHORS_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    }
